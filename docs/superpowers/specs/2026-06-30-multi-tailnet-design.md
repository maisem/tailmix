...
Build a Tailscale fork that can log into multiple tailnets at the same time and make node devices in every active tailnet reachable concurrently from one host.
...
Each forked `tsnet` profile engine owns:
...
5. The packet enters the selected forked `tsnet` engine's packet path.
...
There is no compatibility mode. The fork exposes a new multi-tailnet-native CLI/API instead of preserving upstream single-tailnet command semantics.
...
The daemon persists multi-tailnet state as first-class data, while each forked `tsnet` profile engine keeps its own tailnet identity state.
...
The design depends on a forked `tsnet` packet-facing API. The implementation must identify the narrowest boundary that lets the daemon provide the shared TUN/multiplexer while keeping each profile engine's control-plane and transport responsibilities intact.
