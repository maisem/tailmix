...
> Adapted from the [Tailscale commit message guidelines](https://github.com/tailscale/tailscale/blob/main/docs/commit-messages.md).
> Tailored for this repository.
...
When you use `git revert` to revert a commit, the default commit message will identify the commit SHA and message that was reverted. You must expand this message to explain **why** it is being reverted.
